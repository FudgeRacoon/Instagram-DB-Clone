...
namespace re
{
    ...
    public:
        ...
        static void RegisterEvent(WindowHandle handle, EventArgs... args)
        {
            EventType* event = new EventType(std::forward<EventArgs>(args)...);

            if(event->GetEventType() == KEY_ACTION)
            {
            ...
            if(event->GetEventType() == MOUSE_ACTION)
            {   
